...
class TestDempster(unittest.TestCase):
    ...
    def test_can_do_dempster(self):
        ...
        """
        _equal(m1m2.get_measure('A'), 0)
        _equal(m1m2.get_measure('A', 'C'), 0.32)
        _equal(m1m2.get_measure('C'), 0.6)
        _equal(m1m2.get_measure('B'), 0)
        ...
        _equal(m1m2.get_plausibility(subjectA, subjectB), 0.32)
        _equal(m1m2.get_plausibility(subjectA, subjectC),
                         0.48 + 0.32 + 0.12)
        ...
        _equal(m1m2m3.get_measure('A'), 0)
        _equal(m1m2m3.get_measure('A', 'C'), 0.16*correct_factor)
        _equal(m1m2m3.get_measure('C'), 0.3*correct_factor)
        _equal(m1m2m3.get_measure('B'), 0.04*correct_factor)
